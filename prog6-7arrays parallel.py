...
testLetter = [""] *(NUM_TESTS + 1) #initial strig array to NULL string (two "" in a row)
for testNum in range(1,NUM_TESTS + 1):
    test[testNum] = int(input("Test "+ str(testNum) + ": "))
    testLetter[testNum]=determineGrade(test[testNum])
for testNum in range(1,NUM_TESTS + 1):
    print("test ",testNum," score is ",test[testNum]," letter grade ",testLetter[testNum],sep="")
...
for i in range(2,NUM_TESTS + 1):  #why start index at 2?
     if test[i] > max:
         max = test[i]
     if test[i] < min:
         min = test[i]
